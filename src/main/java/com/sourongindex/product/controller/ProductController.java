...
public class ProductController {
	...
	@RequestMapping("/edit")
	public String edit(Integer productid,ModelMap map){
		if(productid!=null){
	...
	public String doEdit(ProductVO entity,Integer productid,@RequestParam("pic")MultipartFile file) throws IOException{
		String saveImage=ConfigUtil.getValue("saveImage");
		if(entity.getProductid()!=null){//修改
			ProductVO prvo=service.get(productid);
			String icon=prvo.getIcon();
			if(icon!=null){
				new File(ConfigUtil.getValue("saveImage")+icon).delete();//删除原先的图片
			}
			if(!file.isEmpty()){
	...
	public @ResponseBody JsonResult doDelete(Integer productid){
